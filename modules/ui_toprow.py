...
class Toprow:
    ...
    def create_prompts(self):
        with gr.Column(elem_id=f"{self.id_part}_prompt_container", elem_classes=["prompt-container-compact"] if self.is_compact else [], scale=6):
            ...
            # ChatGPT Enhancement Row
            with gr.Row(elem_id=f"{self.id_part}_chatgpt_row", elem_classes=["chatgpt-row"]):
                with gr.Column(scale=1, min_width=80):
                    self.enhance_prompt_btn = gr.Button("✨ Enhance Prompt", elem_id=f"{self.id_part}_enhance_prompt", elem_classes=["chatgpt-btn"], size="sm")
                with gr.Column(scale=2, min_width=220):
                    self.enhancement_percentage = gr.Slider(
                        minimum=10,
                        maximum=100,
                        step=10,
                        value=50,
                        label="Enhancement %",
                        elem_id=f"{self.id_part}_enhancement_percentage",
                        elem_classes=["enhancement-slider"]
                    )
                with gr.Column(scale=2, min_width=200):
                    self.style_preference = gr.Dropdown(
                        choices=["photorealistic", "artistic", "anime", "digital art", "oil painting", "watercolor", "sketch", "3D render"],
                        value="photorealistic",
                        label="Style",
                        elem_id=f"{self.id_part}_style_preference",
