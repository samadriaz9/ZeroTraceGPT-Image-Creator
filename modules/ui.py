...
from modules import sd_hijack, sd_models, script_callbacks, ui_extensions, deepbooru, extra_networks, ui_common, ui_postprocessing, progress, ui_loadsave, shared_items, ui_settings, timer, sysinfo, ui_checkpoint_merger, scripts, sd_samplers, processing, ui_extra_networks, ui_toprow, launch_utils, chatgpt_integration
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Tab("Generation", id="txt2img_generation") as txt2img_generation_tab, ResizeHandleRow(equal_height=False):
            ...
            # ChatGPT Integration Connections
            if hasattr(toprow, 'enhance_prompt_btn'):
                toprow.enhance_prompt_btn.click(
                    fn=wrap_queued_call(chatgpt_integration.enhance_prompt_ui),
                    inputs=[toprow.prompt, toprow.style_preference, toprow.enhancement_percentage],
                    outputs=[toprow.prompt],
                    show_progress=True
                )
            
            # Improve image button removed as requested
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Tab("Generation", id="img2img_generation") as img2img_generation_tab, ResizeHandleRow(equal_height=False):
            ...
            # ChatGPT Integration Connections for img2img
            if hasattr(toprow, 'enhance_prompt_btn'):
                toprow.enhance_prompt_btn.click(
                    fn=wrap_queued_call(chatgpt_integration.enhance_prompt_ui),
                    inputs=[toprow.prompt, toprow.style_preference, toprow.enhancement_percentage],
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="ZeroTraceGPT Image Creator") as demo:
        # Add header with logo
        with gr.Row(elem_id="header_row"):
            with gr.Column(scale=1):
                gr.HTML(f"""
                <div style="display: flex; align-items: center; gap: 2rem; padding: 2rem;">
                    <img src="file/logo.png" alt="ZeroTraceGPT Logo" style="height: 80px; width: auto; border-radius: 16px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3); border: 3px solid #1e40af;">
                    <div>
                        <h1 style="margin: 0; background: linear-gradient(135deg, #1e40af, #1e3a8a); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 3rem; font-weight: 900; text-shadow: 0 4px 8px rgba(0, 0, 0, 0.3);">ZeroTraceGPT</h1>
                        <p style="margin: 0; color: #1e293b; font-size: 1.4rem; font-weight: 700; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2);">AI Image Creator</p>
                    </div>
                </div>
                """)
        ...
        footer = footer.format(versions=versions_html(), api_docs="/docs" if shared.cmd_opts.api else "https://github.com/samadriaz9/ZeroTraceGPT-Image-Creator/wiki/API")
...
def versions_html():
    ...
    return f"""
version: <a href="https://github.com/samadriaz9/ZeroTraceGPT-Image-Creator/commit/{commit}">{tag}</a>
&#x2000;•&#x2000;
